...
class InputHandler {
  ...
  InputHandler(this._terminal) {
    ...
    _parser.setCsiHandler ('b', (pars, collect) => _repeatPrecedingCharacter (pars));
    _parser.setCsiHandler ('c', (pars, collect) => _terminal.csiDA1 (pars, collect));
    ...
    _parser.setCsiHandler ('n', (pars, collect) => _terminal.csiDSR (pars, collect));
    ...
    _parser.setCsiHandler ('q', (pars, collect) => _setCursorStyle (pars, collect));
    ...
    _parser.setCsiHandler ('v', (pars, collect) => _terminal.csiDECCRA (pars, collect));
    ...
    _parser.setExecuteHandler (11, _terminal.lineFeedBasic);   // VT Vertical Tab - ignores auto-new-line behavior in ConvertEOL
    ...
    _parser.setEscHandler ('(' + flag, (code, f) => _selectCharset ('(' + flag));
    _parser.setEscHandler (')' + flag, (code, f) => _selectCharset (')' + flag));
    _parser.setEscHandler ('*' + flag, (code, f) => _selectCharset ('*' + flag));
    _parser.setEscHandler ('+' + flag, (code, f) => _selectCharset ('+' + flag));
    _parser.setEscHandler ('-' + flag, (code, f) => _selectCharset ('-' + flag));
    _parser.setEscHandler ('.' + flag, (code, f) => _selectCharset ('.' + flag));
    _parser.setEscHandler ('/' + flag, (code, f) => _selectCharset ('/' + flag)); // TODO: supported?
    }
  ...
  var scrollBottomAbsolute = _terminal.rows - 1 + buffer.yBase - scrollBottomRowsOffset + 1;
  ...
  // bold text
  flags |= Flags.Bold.value;
  } else if (p == 3) {
  // italic text
  flags |= Flags.Italic.value;
  } else if (p == 4) {
  // underlined text
  flags |= Flags.Underline.value;
  } else if (p == 5) {
  // blink
  flags |= Flags.Blink.value;
  } else if (p == 7) {
  // inverse and positive
  // test with: echo -e '\e[31m\e[42mhello\e[7mworld\e[27mhi\e[m'
  flags |= Flags.Inverse.value;
  } else if (p == 8) {
  // invisible
  flags |= Flags.Invisible.value;
  } else if (p == 2) {
  // dimmed text
  flags |= Flags.Dim.value;
  } else if (p == 22) {
  // not bold nor faint
  flags &= ~Flags.Bold.value;
  flags &= ~Flags.Dim.value;
  ...
  var buffer = _terminal.buffer;
  buffer.y = p - 1;
  if (buffer.y >= _terminal.rows)
  buffer.y = _terminal.rows - 1;

  buffer.x = q - 1;
  if (buffer.x >= _terminal.cols)
  buffer.x = _terminal.cols - 1;
  }
  ...
  buffer.x,
  buffer.x + p,
  new CharData (_terminal.eraseAttr ()));
  }
  ...
  var bytesRemaining = _readingBuffer.bytesLeft;
  ...
  if (str != null) {
  ch = str [0].runes.first;
  code = ch;
  }
  }
  if (screenReaderMode)
  _terminal.emitChar (ch);
  ...
  bufferRow!.insertCells (buffer.x, chWidth, _terminal.marginMode ? buffer.marginRight : cols - 1, empty);
  ...
  var charData = CharData (curAttr, rune: String.fromCharCode(code), width: chWidth, code: ch);
