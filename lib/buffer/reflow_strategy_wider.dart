...
class ReflowStrategyWider extends ReflowStrategy {
  ...
  void reflow(int newCols, int newRows, int oldCols, int oldRows) {
    final toRemove = _getLinesToRemove(
        buffer.lines, oldCols, newCols, buffer.yBase + buffer.y, CharData.nul);
  ...
  List<int> _getLinesToRemove(CircularList<BufferLine> lines, int oldCols,
      int newCols, int bufferAbsoluteY, CharData nullCharacter) {
    ...
    for (int y = 0; y < lines.length - 1; y++) {
      ...
      while (srcLineIndex < wrappedLines.length) {
        ...
        wrappedLines[destLineIndex].copyFromRange(
        ...
        if (destCol == 0 && destLineIndex != 0) {
          if (wrappedLines[destLineIndex - 1].widthAt(newCols - 1) == 2) {
            wrappedLines[destLineIndex].copyFromRange(
                wrappedLines[destLineIndex - 1], newCols - 1, destCol++, 1);
            // Null out the end of the last row
            wrappedLines[destLineIndex - 1]
                .replaceCells(newCols - 1, 1, nullCharacter);
          }
        }
      }
      ...
      for (int ix = wrappedLines.length - 1; ix > 0; ix--) {
        if (ix > destLineIndex || wrappedLines[ix].trimmedLength == 0) {
          countToRemove++;
        } else {
          break;
        }
  ...
  void _adjustViewport(int newCols, int newRows, int countRemoved) {
    ...
    while (viewportAdjustments-- > 0) {
      if (buffer.yBase == 0) {
        ...
        if (buffer.lines.length < newRows) {
          ...
          buffer.lines.push(new BufferLine(newCols, CharData.nul));
        }
      } else {
    ...
    buffer.savedY = max(buffer.savedY - countRemoved, 0);
  }
}
